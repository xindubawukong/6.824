...
logs=$(find ./tmp -maxdepth 1 -name 'test-*.log' -type f -printf '.' | wc -c)
success=$(grep -E '^PASS$' ./tmp/test-*.log | wc -l)
((failed = logs - success))
	...
	if ! wait "$1"; then
		if command -v notify-send >/dev/null 2>&1 &&((failed == 0)); then
			notify-send -i weather-storm "Tests started failing" \
				"$(pwd)\n$(grep FAIL: -- ./tmp/*.log | sed -e 's/.*FAIL: / - /' -e 's/ (.*)//' | sort -u)"
		fi
		((failed += 1))
	else
		((success += 1))
...
cleanup() {
	for pid in "${waits[@]}"; do
		kill "$pid"
		wait "$pid"
		rm -rf "./tmp/test-${is[0]}.err" "./tmp/test-${is[0]}.log"
		is=("${is[@]:1}")
	done
	exit 0
}
...
for i in $(seq "$((success+failed+1))" "$runs"); do
	...
	if [[ -z "$test" ]]; then
		./tester.so -test.v 2> "./tmp/test-${i}.err" > "./tmp/test-${i}.log" &
		pid=$!
	else
		./tester.so -test.run "$test" -test.v 2> "./tmp/test-${i}.err" > "./tmp/test-${i}.log" &
		pid=$!
	fi
